...
class EdgeSkin extends PlayerxEdgeClassSkinAbstract {
    /**
     * Skin constructor. Hooks to playerx_edge_admin_scripts_init and playerx_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('playerx_edge_action_admin_scripts_init', array($this, 'registerStyles'));
        add_action('playerx_edge_action_admin_scripts_init', array($this, 'registerScripts'));

        add_action('playerx_edge_action_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('playerx_edge_action_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('playerx_edge_action_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('playerx_edge_action_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			playerx_edge_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            playerx_edge_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = playerx_edge_get_admin_tab();
        $active_page = $playerx_edge_global_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see PlayerxEdgeClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param PlayerxEdgeClassAdminPage $page current page to load
     * @see PlayerxEdgeClassSkinAbstract::loadTemplatePart()
     */

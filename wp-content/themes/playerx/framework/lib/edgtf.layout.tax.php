...
/*
   Class: PlayerxEdgeClassTaxonomyField
   A class that initializes PlayerxEdgeClass Taxonomy Field
*/

class PlayerxEdgeClassTaxonomyField implements iPlayerxEdgeInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'playerx_edge_filter_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
...
abstract class PlayerxEdgeClassTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class PlayerxEdgeClassTaxonomyFieldText extends PlayerxEdgeClassTaxonomyFieldType {
...
class PlayerxEdgeClassTaxonomyFieldImage extends PlayerxEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
            <div class="form-field">
                <label for="<?php echo esc_attr( $name ); ?>"><?php echo esc_html( $label ); ?></label>
                ...
                <p>
                    <input type="button" class="button button-secondary edgtf-tax-media-add" name="edgtf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'playerx' ); ?>"/>
                    <input type="button" class="button button-secondary edgtf-tax-media-remove" name="edgtf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'playerx' ); ?>"/>
                </p>
                <input type="hidden" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $name ); ?>" class="edgtf-tax-custom-media-url" value="">
            ...
            <tr class="form-field" <?php if ( $hidden ) { ?> style="display: none"<?php } ?>>
                ...
                <td>
                    ...
                    <p>
                        <input type="button" class="button button-secondary edgtf-tax-media-add" name="edgtf-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'playerx' ); ?>"/>
                        <input data-termid="<?php echo esc_attr( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>" type="button" class="button button-secondary edgtf-tax-media-remove" name="edgtf-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'playerx' ); ?>"/>
                    </p>
                    <input type="hidden" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $name ); ?>" class="edgtf-tax-custom-media-url" value="">
					<?php wp_nonce_field( 'edgtf_tax_del_image_nonce', 'edgtf_tax_del_image_nonce' ); ?>
                </td>
...
class PlayerxEdgeClassTaxonomyFieldSelect extends PlayerxEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class PlayerxEdgeClassTaxonomyFieldIcon extends PlayerxEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
                ...
                <div class="form-field edgt-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                    <label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'playerx' ); ?></label>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
                ...
                <tr class="form-field edgt-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                    <th scope="row"><?php esc_html_e( 'Icon', 'playerx' ); ?></th>
                    <td>
                        <select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons      = playerx_edge_icon_collections()->getIconCollection( $icons_collection );
...
class PlayerxEdgeClassTaxonomyFieldColor extends PlayerxEdgeClassTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class PlayerxEdgeClassTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {

		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new PlayerxEdgeClassTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;

			case 'image':
				$field = new PlayerxEdgeClassTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;

			case 'selectblank':
				$field = new PlayerxEdgeClassTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;

			case 'icon':
				$field = new PlayerxEdgeClassTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;

			case 'color':
				$field = new PlayerxEdgeClassTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;

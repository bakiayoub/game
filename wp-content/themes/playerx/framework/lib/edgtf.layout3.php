...
/*
   Class: PlayerxEdgeClassMultipleImages
   A class that initializes Edge Multiple Images
*/

class PlayerxEdgeClassMultipleImages implements iPlayerxEdgeInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$playerx_edge_global_Framework->edgtMetaBoxes->addOption( $this->name, "" );
	}
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="edgtf-gallery-uploader">
                                <a class="edgtf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'playerx' ); ?></a>
                                <a class="edgtf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'playerx' ); ?></a>
                            </div>
...
class PlayerxEdgeClassTwitterFramework implements iPlayerxEdgeInterfaceRender {
	public function render( $factory ) {
		$twitterApi = PlayerxTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'playerx' );
			} else {
				$message = $responseObj->message;
			}
		}

		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'playerx' ) : esc_html__( 'Connect with Twitter', 'playerx' );
		?>
		<?php if ( $message !== '' ) { ?>
        ...
        <div class="edgtf-page-form-section" id="edgtf_enable_social_share">
            <div class="edgtf-field-desc">
                <h4><?php esc_html_e( 'Connect with Twitter', 'playerx' ); ?></h4>
                <p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'playerx' ); ?></p>
            </div>
            <div class="edgtf-section-content">
                <div class="container-fluid">
                    <div class="row">
...
class PlayerxEdgeClassInstagramFramework implements iPlayerxEdgeInterfaceRender {
	public function render( $factory ) {
		$instagram_api = PlayerxInstagramApi::getInstance();
		$message       = '';
		...
		if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'playerx' );
		}
		...
		if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'playerx' );
		}
		...
		if ( ! empty( $_GET['disconnect'] ) ) {
			...
			$message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'playerx' );
		...
		<div class="edgtf-page-form-section" id="edgtf_enable_social_share">
			<div class="edgtf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'playerx' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'playerx' ); ?></p>
			</div>
			<div class="edgtf-section-content">
				<div class="container-fluid">
					<?php
					...
					if ($instagram_user_id) { ?>
						<div class="row">
							<div class="col-lg-12">
								<p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'playerx' ); echo esc_attr($instagram_user_id) ?></p>
							</div>
						</div>
					<?php } ?>
					<div class="row">
						<?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'playerx' ); ?></a>
							</div>
						<?php } else if ( '' === $connection_type  ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'playerx' ); ?></a>
							</div>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Business account', 'playerx' ); ?></a>
							</div>
						<?php } else { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'playerx' ) ?></a>
							</div>
						<?php } ?>
					</div>
				</div>
...
class PlayerxEdgeClassRepeater implements iPlayerxEdgeInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'playerx' );
			...
			<?php if ( $this->table_layout ) { ?>
                <div class="edgtf-repeater-table-heading">
                    <div class="edgtf-repeater-fields-holder">
                        <div class="edgtf-repeater-table-cell edgtf-repeater-sort"><?php esc_html_e( 'Order', 'playerx' ) ?></div>
                        ...
                        <div class="edgtf-repeater-table-cell edgtf-repeater-remove"><?php esc_html_e( 'Remove', 'playerx' ) ?></div>
                    </div>
                </div>
			<?php } ?>
                                    ...
                                    <div class="edgtf-repeater-fields-row <?php echo esc_attr( $col_width_class ); ?>">
                                        <div class="edgtf-repeater-fields-row-inner">
											...
											if ( $field['type'] == 'repeater' ) {
                                                                            ...
                                                                            <div class="edgtf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
                                                                                <div class="edgtf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = playerx_edge_return_repeater_dependency_options_array( array(
																							'field'        => $field,
                                                                                    ...
                                                                                    <div class="<?php echo esc_attr( $containerClass ); ?>" <?php echo playerx_edge_get_inline_attrs( $data, true ); ?>>
																						<?php
																						$factory->render( $field_inner['type'], $field_inner['name'], $field_inner['label'], $field_inner['description'], $field_inner['options'], $field_inner['args'], array(
																							'name'  => $this->name . '[' . $counter . '][' . $field['name'] . ']',
																							'index' => $counter2,
																							'value' => $repeater_inner_field_value
																						) );
																						?>
                                                                                    </div>
                                                                                </div>
                                                ...
                                                <div class="<?php echo esc_attr( $containerClass ); ?>" <?php echo playerx_edge_get_inline_attrs( $data, true ); ?>>
													<?php
													$factory->render( $field['type'], $field['name'], $field['label'], $field['description'], $field['options'], $field['args'], array(
														'name'  => $this->name,
														'index' => $counter,
                                        ...
                                        <div class="edgtf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
                                            <div class="edgtf-repeater-fields-row-inner">
                                                ...
                                                <div class="<?php echo esc_attr( $containerClass ); ?>" <?php echo playerx_edge_get_inline_attrs( $data, true ); ?>>
													...
													$factory->render( $field_inner['type'], $field_inner['name'], $field_inner['label'], $field_inner['description'], $field_inner['options'], $field_inner['args'], array(
														'name'  => $this->name . '[{{{ data.rowIndex }}}][' . $field['name'] . ']',
														'index' => '{{{ data.rowInnerIndex }}}',
														'value' => $repeater_inner_template_field_value
													) );
													?>
                                                </div>
                                            </div>
...
class PlayerxEdgeClassFieldAddress extends PlayerxEdgeClassFieldType {
                ...
                <div class="container-fluid">
                    <div class="row">
						...
						<?php endif; ?>
                            ...
                            <button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'playerx' ); ?></button>
                            <a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'playerx' ); ?></a>
                        </div>
                    </div>
                </div>
            </div>
...
class PlayerxEdgeClassFieldIcon extends PlayerxEdgeClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = playerx_edge_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = playerx_edge_icon_collections()->getIconCollectionsKeys();
		?>
            ...
            <div class="edgtf-section-content">
                <div class="container-fluid">
                        ...
                        <div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
                            <select name="<?php echo esc_attr( $name ) . '[icon_pack]'; ?>" class="<?php echo esc_attr( $select2 ) ?> form-control edgtf-form-element icon-dependence">
								<?php foreach ( $icon_packs as $key => $value ) {
                                    ...
                                    <option <?php if ( ! empty( $rvalue ) && $rvalue['icon_pack'] == $key ) {
					...
					<?php foreach ( $icons_collections as $icons_collection ) { ?>
						<?php
						$icons_param = playerx_edge_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
						$style       = 'display: none';
                        ...
                        <div class="row edgtf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                            <div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" class="<?php echo esc_attr( $select2 ) ?> form-control edgtf-form-element">
									<?php
									$icons       = playerx_edge_icon_collections()->getIconCollection( $icons_collection );
									$active_icon = $rvalue[ $icons_param ];
									foreach ( $icons->icons as $option => $key ) { ?>
                                        <option value="<?php echo esc_attr( $option ); ?>" <?php if ( $option == $active_icon ) {
											echo 'selected';

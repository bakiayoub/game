...
if ( ! function_exists( 'playerx_edge_get_icon_pack_collections' ) ) {
	function playerx_edge_get_icon_pack_collections() {
		$options = apply_filters( 'playerx_edge_filter_add_icon_pack_into_collection', $options = array() );

		return $options;
	}

	add_action( 'after_setup_theme', 'playerx_edge_get_icon_pack_collections' );
}

if ( ! function_exists( 'playerx_edge_get_icon_pack_options' ) ) {
	function playerx_edge_get_icon_pack_options() {
		$options = apply_filters( 'playerx_edge_filter_add_icon_pack_into_options', $options = array() );

		return $options;
	}

	add_action( 'after_setup_theme', 'playerx_edge_get_icon_pack_options' );
}

if ( ! function_exists( 'playerx_edge_get_icon_pack_social_options' ) ) {
	function playerx_edge_get_icon_pack_social_options() {
		$options = apply_filters( 'playerx_edge_filter_add_icon_pack_into_social_options', $options = array() );

		return $options;
	}

	add_action( 'after_setup_theme', 'playerx_edge_get_icon_pack_social_options' );
}

/*
 * Class: PlayerxEdgeClassIconCollections
 */
...
class PlayerxEdgeClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = playerx_edge_get_icon_pack_collections( true );
		$this->iconPackOptions       = playerx_edge_get_icon_pack_options();
		$this->iconPackSocialOptions = playerx_edge_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = playerx_edge_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'playerx' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'playerx' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'playerx' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'playerx' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'playerx' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'playerx' ), $n )
			);

			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'playerx' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'playerx' )       => 'fa-lg',
			esc_html__( 'Small', 'playerx' )      => 'fa-2x',
			esc_html__( 'Medium', 'playerx' )     => 'fa-3x',
			esc_html__( 'Large', 'playerx' )      => 'fa-4x',
			esc_html__( 'Very Large', 'playerx' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iPlayerxEdgeInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'playerx' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                       = array();
		$vc_array[ esc_html__( 'No Icon', 'playerx' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'playerx-edge-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}

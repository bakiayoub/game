...
class PlayerxEdgeClassLike {
	private static $instance;

	private function __construct() {
		add_action( 'wp_ajax_playerx_edge_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_playerx_edge_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'playerx' );

		if ( isset( $_COOKIE[ 'edgtf-like_' . $post->ID ] ) ) {
			$class = 'edgtf-like liked';
			$title = esc_html__( 'You already like this!', 'playerx' );
		}
...
if ( ! function_exists( 'playerx_edge_create_like' ) ) {
	function playerx_edge_create_like() {
		PlayerxEdgeClassLike::get_instance();
	}

	add_action( 'after_setup_theme', 'playerx_edge_create_like' );
}
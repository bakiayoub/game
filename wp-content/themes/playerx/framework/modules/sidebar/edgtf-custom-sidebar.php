 ...
 */
if ( ! class_exists( 'PlayerxEdgeClassSidebar' ) ) {
	class PlayerxEdgeClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'playerx' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'playerx-edge-admin-sidebar-script', EDGE_ROOT . '/framework/admin/assets/js/edgtf-ui/edgtf-sidebar.js' );
			wp_enqueue_style( 'playerx-edge-admin-sidebar', EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_attr__( 'Enter Name of the new Widget Area', 'playerx' ) . "' name='edgtf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_attr__( 'Add Widget Area', 'playerx' ) . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'playerx_edge_filter_custom_widget_args', $args );

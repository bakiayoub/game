...
if ( ! function_exists( 'playerx_edge_set_title_breadcrumbs_type_for_options' ) ) {
	/**
	 * This function set breadcrumbs title type value for title options map and meta boxes
	 */
	function playerx_edge_set_title_breadcrumbs_type_for_options( $type ) {
		$type['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'playerx' );
		
		return $type;
	}
	
	add_filter( 'playerx_edge_filter_title_type_global_option', 'playerx_edge_set_title_breadcrumbs_type_for_options' );
	add_filter( 'playerx_edge_filter_title_type_meta_boxes', 'playerx_edge_set_title_breadcrumbs_type_for_options' );
}

if ( ! function_exists( 'playerx_edge_custom_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs
	 */
	function playerx_edge_custom_breadcrumbs() {
		global $post;
		
		$pageid    = playerx_edge_get_page_id();
		$homeLink  = esc_url( home_url( '/' ) );
		$home      = esc_html__( 'Home', 'playerx' );
		...
		$output = '<div itemprop="breadcrumb" class="edgtf-breadcrumbs ' . esc_attr( $holder_classes ) . '" ' . playerx_edge_get_inline_attr( $holder_styles, 'style', ';' ) . '>';
			...
			if ( is_home() && ! is_front_page() ) {
				...
				if ( is_tag() ) {
					$childContent .= $before . esc_html__( 'Posts tagged ', 'playerx' ) . '"' . single_tag_title( '', false ) . '"' . $after;
					...
					$childContent    .= $before . esc_html__( 'Articles posted by ', 'playerx' ) . get_the_author_meta( 'display_name', $author_id ) . $after;
					...
					$childContent .= $before . esc_html__( 'Search results for ', 'playerx' ) . '"' . get_search_query() . '"' . $after;
					
				} elseif ( is_404() ) {
					$childContent .= $before . esc_html__( 'Error 404', 'playerx' ) . $after;
				...
				if ( get_query_var( 'paged' ) ) {
					$childContent .= $before . " (" . esc_html__( 'Page', 'playerx' ) . ' ' . get_query_var( 'paged' ) . ")" . $after;
					
				}
				
				$output .= apply_filters( 'playerx_edge_filter_breadcrumbs_title_child_output', $childContent, $delimiter, $before, $after );
			}
		
		$output .= '</div>';
		
		echo wp_kses( apply_filters( 'playerx_edge_filter_breadcrumbs_title_output', $output ), array(
			'div'  => array(
				'itemprop' => true,
				'id'       => true,
				'class'    => true,
